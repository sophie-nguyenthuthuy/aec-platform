...
// ---------- Tester results ----------
//
// Computes the winning rule per pasted input line. The "winner" is
// the first rule whose pattern matches — same first-match semantics
// the server uses. The rules list is already sorted by priority ASC
// (server default), so the order here matches the production order.
...
function TesterResults({
  ...
  if (inputs.length <= 1) {
    // Single-line mode — the per-rule green-dot highlight in
    // `RulesTable` already conveys the answer; an extra results panel
    // would just duplicate. Hide.
    return null;
  }
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {results.map((row, idx) => (
            <tr key={`${idx}-${row.input}`} className="border-t border-slate-100">
              ...
              <td className="px-3 py-2">
                {row.winner ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded bg-rose-50 px-2 py-0.5 text-xs text-rose-700">
                    {t("no_match")}
                  </span>
                )}
              </td>
