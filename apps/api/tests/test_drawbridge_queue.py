...
async def test_enqueue_ingest_document_falls_back_when_pool_unavailable(monkeypatch):
    ...
    # get_pool raises to simulate Redis unreachable.
    from apps.api.workers import queue

    monkeypatch.setattr(
        queue, "get_pool", AsyncMock(side_effect=RuntimeError("redis down"))
    )
...
@asyncio_test
async def test_enqueue_ingest_document_uses_arq_when_available(
