...
async def test_upload_pdf_skips_thumbnail_and_records_row(client, patch_session, patch_s3, fake_auth):
    body = b"%PDF-1.4\n%fake pdf bytes"
    r = await client.post(
        "/api/v1/files",
        data={"source_module": "drawbridge"},
...
async def test_upload_image_generates_thumbnail(client, patch_session, patch_s3, monkeypatch):
...
async def test_upload_image_falls_back_when_pillow_fails(client, patch_session, patch_s3, monkeypatch):
...
async def test_upload_infers_extension_from_filename(client, patch_session, patch_s3):
...
async def test_upload_with_project_id_forwards_it_to_insert(client, patch_session, patch_s3):
