...
export function ScopeBuilder({ items, onChange }: ScopeBuilderProps) {
  ...
  return (
    <div className="space-y-3">
      {items.map((item, idx) => {
        const deliverablesId = `scope-deliverables-${item.id}`;
        return (
          <div
            key={item.id}
            draggable
          ...
          >
            ...
            <div className="flex-1 space-y-2">
              <div className="flex gap-2">
                <select
                  aria-label={`Phase, scope item ${idx + 1}`}
                ...
                <Input
                  aria-label={`Title, scope item ${idx + 1}`}
                  value={item.title}
              ...
              <Textarea
                aria-label={`Description, scope item ${idx + 1}`}
              ...
              />
              <div>
                <Label htmlFor={deliverablesId} className="text-xs text-muted-foreground">
